...
// Global variables DEFINITION (only once)
...
bool Rook::isValid_Move(int r, int co, Piece* board[8][8])
{
	...
	// Check destination - can't capture own piece
...
bool King::isValid_Move(int r, int co, Piece* board[8][8]) {
	...
	if (rowDiff <= 1 && colDiff <= 1) {
		...
		// Check if moving into check (will be checked by isMoveSafe)
		return true;
	}
	...
	{
		bool kingMoved;
		bool rookMovedKingSide;
		bool rookMovedQueenSide;

		if (getColor() == "white")
		{
			kingMoved = whiteKingMoved;
			rookMovedKingSide = whiteRookHMoved;
			rookMovedQueenSide = whiteRookAMoved;
		}
		else
		{
			kingMoved = blackKingMoved;
			rookMovedKingSide = blackRookHMoved;
			rookMovedQueenSide = blackRookAMoved;
		}
...
void makeMove(Piece* board[8][8], int fromRow, int fromCol, int toRow, int toCol)
{
	...
	if (canPromote(board, toRow, toCol))
	{
		...
		promotePawn(board, toRow, toCol, choice, pieceColor);
	}
}
...
bool isInCheck(string color, Piece* board[8][8]) {
	...
	if (kRow == -1) return false; // Safety check
...
bool isMoveSafe(int fromR, int fromC, int toR, int toC, string color, Piece* board[8][8])
{
	...
	// Safety check
	if (originalSource == nullptr) return false;
	...
	bool kingMoved, bool rookMoved)
{
	...
	// Squares between king and rook must be empty (columns 5 and 6 for white)
	...
	bool kingMoved, bool rookMoved)
{
	...
	// Check all squares between king and rook (columns 1,2,3)
	if (board[row][col - 1] != nullptr) return false;
	if (board[row][col - 2] != nullptr) return false;
...
void doCastleKingSide(Piece* board[8][8], int row)
{
	...
	// Update positions
	if (board[row][6] != nullptr) board[row][6]->setPosition(row, 6);
	if (board[row][5] != nullptr) board[row][5]->setPosition(row, 5);
}
...
void doCastleQueenSide(Piece* board[8][8], int row)
{
	...
	// Update positions
	if (board[row][2] != nullptr) board[row][2]->setPosition(row, 2);
	if (board[row][3] != nullptr) board[row][3]->setPosition(row, 3);
}
